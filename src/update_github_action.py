...
if __name__ == "__main__":
    ...
    t1 = time.time()
    data_path = os.path.normpath(os.path.join(base_path, "..", "data", fileyear))
    ...
    if Datenstand >= dt.datetime.strptime("2021-08-26", "%Y-%m-%d"):
        ...
        path = os.path.normpath(os.path.join(base_path, "..", "dataStore", "agegroup"))
    ...
    path = os.path.normpath(os.path.join(base_path, "..", "dataStore", "cases"))
    os.makedirs(path, exist_ok=True)
    ut.write_json(LK, "districts.json.xz", path)
    ut.write_json(BL, "states.json.xz", path)
    aktuelleZeit = dt.datetime.now().strftime(format="%Y-%m-%dT%H:%M:%SZ")
    ...
    path = os.path.normpath(os.path.join(
        base_path, "..", "dataStore", "historychanges"
    ))
    os.makedirs(path, exist_ok=True)
    pathBL = os.path.join(path, "BL_BaseData.feather")
    pathLK = os.path.join(path, "LK_BaseData.feather")
    ...
    path = os.path.normpath(os.path.join(base_path, "..", "dataStore", "history"))
    os.makedirs(path, exist_ok=True)
    ...
    path = os.path.normpath(os.path.join(base_path, "..", "dataStore", "frozen-incidence"))
    os.makedirs(path, exist_ok=True)
    
    if Datenstand >= dt.datetime.strptime("2023-04-18", "%Y-%m-%d"):
        if os.path.exists(os.path.join(path, "LK.json.xz")):
